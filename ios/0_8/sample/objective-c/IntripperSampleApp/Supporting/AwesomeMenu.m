...
- (void)AwesomeMenuItemDoubleTap:(AwesomeMenuItem *)item
{
    ...
    if ([_delegate respondsToSelector:@selector(awesomeMenuDoubleTap:didSelectIndex:)])
    {
        [_delegate awesomeMenuDoubleTap:self didSelectIndex:item.tag - 1000];
    }
}
...
- (void)_setMenu {
    ...
    for (int i = 0; i < count; i ++)
    {
        ...
        */
        ...
        //Updated logic with k10 Suggestion top to bottom
        //CGPoint farPoint = CGPointMake(startPoint.x,startPoint.y+(menuDistance*(i+1)));
        //Right to left
        CGPoint farPoint;// = CGPointMake(startPoint.x-(menuDistance*(i+1)),startPoint.y);// Right to left horizontal
        //farPoint = CGPointMake(startPoint.x,startPoint.y-(menuDistance*(i+1)));// bottom to top vertical
        farPoint = CGPointMake(startPoint.x,startPoint.y+(menuDistance*(i+1)));// top to Bottom vertical
        
        item.farPoint = farPoint;
        item.endPoint=farPoint;
        item.nearPoint=farPoint;
        item.center = item.startPoint;
        item.delegate = self;
...
- (void)setExpanding:(BOOL)expanding
{
    ...
    // rotate add button
    float angle = 0;//self.isExpanding ? -M_PI_4 : 0.0f;
